...
export function buildDays(
  ...
  now = Date.now(),
  /**
   * Дні, які не згортаються, хоч би й порожні: краї тривалих подій. Тривале не
   * займає рядків у днях, але день, де воно почалось чи закінчилось, мусить
   * лишитись видимим — інакше підпис «▮ ЧЕРЕМША · СЕЗОН ПОЧАВСЯ» нема куди
   * покласти, і початок сезону зникає в «тиша · 6 дн.».
   */
  keepDays: Set<number> = new Set(),
): CalendarRow[] {
  ...
  for (let i = 0; i < days; i++) {
    ...
    if (dayEvents.length === 0 && at !== dayStart(now) && !keepDays.has(at)) {
