...
describe('тост після застосування', () => {
  ...
  // П4-Т4. Тост профілю ігнорував appliedCount і повертав захардкоджений
  // рядок завжди. 7 вересня людина побачила підтвердження запису, якого не
  // було: гілка застосування вміє лише kind 'member', модель прислала три
  // операції іншої форми, лягло нуль — а тост сказав «Записано».
  //
  // Рядок «Записано в „Про тебе"» лишився захардкодженим і це правильно:
  // профіль не рахують позиціями. Але тепер він за guard'ом на нуль, тобто
  // звучить тільки тоді, коли справді записано.
  it('профіль при нулі не каже «Записано»', () => {
    expect(appliedToast({ type: 'profile', ops: [{}, {}, {}] }, 0)).not.toContain('Записано');
    expect(appliedToast({ type: 'profile', ops: [{}, {}, {}] }, 0)).toBe('У „Про тебе" нічого не змінилось');
    // Картка ПОЛЯ профілю — інша гілка застосування, той самий тост.
    expect(appliedToast({ type: 'profile', field: 'no', mode: 'append', text: '' }, 0))
      .not.toContain('Записано');
  });
