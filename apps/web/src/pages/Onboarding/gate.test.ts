...
import { shouldShowOnboarding, onboardingRuleWants, markSeenLocally, ONBOARDING_SEEN_KEY, ONBOARDING_ENABLED } from './Onboarding';

const me = (id: string, seen: string | null = null) => ({ user: { id, welcome_seen_at: seen } });

beforeEach(() => { localStorage.clear(); });

describe('правило «чи бачив» (кеш проти сервера)', () => {
  it('сервер каже «не бачив», кеша немає — показуємо', () => {
    expect(onboardingRuleWants(me('u1'))).toBe(true);
  });

  it('сервер каже «бачив» — не показуємо, хай кеш мовчить', () => {
    expect(onboardingRuleWants(me('u1', '2026-09-01T10:00:00Z'))).toBe(false);
  });
  ...
  it('кеш ТІЄЇ САМОЇ людини притримує: вона щойно дійшла до кінця, сервер ще не знає', () => {
    ...
    markSeenLocally('u1');
    expect(onboardingRuleWants(me('u1'))).toBe(false);
  });

  it('порожній id нічого не підтверджує', () => {
    markSeenLocally('');
    expect(onboardingRuleWants(me(''))).toBe(true);
  });
});

// 14.09, власник: «відключи онбординг поки що». Вимикач — одна константа;
// правило вище лишається живим, щоб увімкнути назад однією зміною.
describe('вимикач онбордингу', () => {
  it('ONBOARDING_ENABLED = false: новий акаунт без кешу — НЕ показуємо', () => {
