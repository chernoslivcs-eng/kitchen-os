...
/** «свіже до 8 вер · ще 2 дні» / «сьогодні» / «термін вийшов» / «без терміну». */
export function freshLine(b: Pick<PantryBatch, 'expires_at' | 'days'>): string {
  if (!b.expires_at || b.days == null) return 'без терміну';
  const until = `свіже до ${shortDate(b.expires_at)}`;
  if (b.days > 0) return `${until} · ще ${b.days} ${plural(b.days, ['день', 'дні', 'днів'])}`;
  if (b.days === 0) return `${until} · сьогодні`;
  return `${until} · термін вийшов`;
}
