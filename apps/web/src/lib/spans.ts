...
export function edgeCaption(e: EventOccurrence, at: number): string | null {
  ...
  const kindWord = e.kind === 'season' ? 'сезон'
    : e.kind === 'tradition' ? 'традиція'
    : e.force === 'restrict' ? 'обмеження'
    : e.kind === 'supply' ? 'завіз'
    : 'подія';
  // Формат бандла (Screens D3b, пігулка зі знаком роду): у день старту —
  // «Гарбуз · сезон · до ≈ 30.11», у день кінця — «Черемша · кінець». Дата
  // кінця в старті — щоб день старту відповідав на «доки», не змушуючи
  // відкривати подію. Знак «▮» зняв v3: рід тепер несе іконка пігулки.
  const until = new Date(e.end).toLocaleDateString('uk-UA', { day: '2-digit', month: '2-digit' });
  if (d === dayStart(e.start) && d === dayStart(e.end)) return `${e.title} · ${kindWord}`;
  if (d === dayStart(e.start)) return `${e.title} · ${kindWord} · до ${e.approx ? '≈ ' : ''}${until}`;
  if (d === dayStart(e.end)) return `${e.title} · кінець`;
  return null;
}
