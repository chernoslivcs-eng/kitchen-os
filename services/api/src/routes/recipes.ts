...
export function recipesRoutes(app: FastifyInstance, repo: Repo) {
  ...
  app.post<{
    ...
    if (call.recipe) {
      ...
      const { avoid } = recipeVetoHits(resolveRecipeLabels(call.recipe, pantry), vetoIndex, (e) => incident({ repo, req }, 'guard', e.event, { user_id: ctx.user_id, ...e }), `${title} ${context ?? ''}`);
      if (avoid.length) {
    ...
    if (bad.length) incident({ repo, req }, 'guard', 'recipe-ing-schema-violation', { user_id: ctx.user_id, bad, title });
