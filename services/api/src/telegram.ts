...
import { resolveRecipeLabels, applyCard, dismissCard, undoCard, signInWithTelegram, getOrCreateTelegramWebToken, attachTelegramLoginUser, helpTopicById, type Repo, type Card, type Recipe, type AttachmentKind, type Tradition } from '@kitchen/domain';
...
export async function handleTelegramFile(deps: TelegramDeps, u: IncomingFile): Promise<TelegramReply> {
  ...
  try {
    ...
    // Власник 20.09 (жива перевірка на проді): фото — ЗАВЖДИ одразу в комору, як текст і
    // голос (серія «наповнюю комору» 19.09 прибрана). Не-зображення (PDF тощо) — як було, pending.
    const auto = kind === 'image';
    const out = await turn({
      user: { user_id: linked.user_id, household_id }, text: u.caption?.trim() || undefined,
      attachments: [{ id }], channel: 'telegram', attachmentApply: auto ? 'auto' : 'pending', host, log,
    });
    const card = out.card;
...
export async function handleTelegramCallback(deps: TelegramDeps, u: IncomingCallback): Promise<{ status: string } | null> {
  ...
  try {
    ...
    if (m[1] === 'undo') {
      // «Скасувати» під авто-карткою фото — той самий undo, що у вебі (токен на картці).
      const pc = await deps.repo.getPending(m[2]!);
      if (pc?.undo_token) await undoCard(deps.repo, m[2]!, pc.undo_token, linked.user_id);
      return { status: COPY.undone };
    }
