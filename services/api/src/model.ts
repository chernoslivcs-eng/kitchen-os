...
export async function callAttachmentParse(atts: AttachmentPayload[]): Promise<AttachmentCall> {
  ...
  // Е2-Т1. Компактний JSON — 6 119 токенів проти 8 629 фактичних на живій
  // картці з 79 позицій (−29% виходу і −29% часу на найдовшій операції
  // продукту). Число з переформатування ТОГО САМОГО обʼєкта, тож воно —
  // ціна повного вмісту компактно, а не результат прогону «стисни».
  //
  // Тому друге речення тут не ввічливість, а запобіжник: пробіли стискає
  // не код, а модель, і модель, якій сказали «коротше», коротшає не лише
  // в пробілах — «спагетті» замість «спагетті Barilla №3». 77% партій із
  // чеків резолвляться в каталог ПО НАЗВІ; коротша назва — порожні теги
  // алергенів — вето, яке мовчить там, де мало б попередити.
  //
  // Фраза стоїть у user-повідомленні, за кеш-межею: `prompt_hash` не
  // міняється, кеш не інвалідується.
  //
  // ДУБЛЬ: та сама фраза дослівно в `packages/eval/model-client.ts`
  // (eval не імпортує api). Правити треба обидва місця, інакше еваль
