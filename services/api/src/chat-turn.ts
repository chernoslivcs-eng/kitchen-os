// Хід чату як функція (Р149): те саме, що POST /v1/chat, але без Fastify —
// Telegram-бот (telegram.ts) кличе його напряму для привʼязаної людини. Тіло
// перенесено з routes/chat.ts дослівно; змінилось лише: вхід — `input` замість
// req/reply; HTTP-помилки — виняток ChatTurnHttpError (маршрут перетворює на
// reply.code().send(), бот — на текст); раковина інцидентів — host з input;
// кожен saveMessage дописує channel, коли хід не з вебу.
//
...
import type { RetailCartAttempt, RetailSearchAttempt } from './routes/retail.js';
...
export interface ChatTurnInput {
  user: { user_id: string; household_id: string };
  text?: string;
...
export async function runChatTurn(repo: Repo, store: AttachmentStore, opts: ChatRouteOpts, input: ChatTurnInput): Promise<ChatTurnOutput> {
  const sink = { repo, req: input.host };
  const saveMsg = (m: MessageRow) => repo.saveMessage(input.channel && input.channel !== 'web' ? { ...m, channel: input.channel } : m);
    ...
    if (attachments?.length) {
      ...
      try {
        ...
        incident(sink, 'broke', 'attachment-model-call-failed', { user_id, household_id, session_id: session.id, err: String(err) });
        throw new ChatTurnHttpError(502, { error: 'model_unavailable' });
      }
      ...
      if (!call.card && looksLikeModelDebris(call.reply ?? '')) {
        incident(sink, 'guard', 'attachment-reply-debris', { user_id, household_id, session_id: session.id });
      ...
      if (call.card?.type === 'intake_diff' && card_id && (input.attachmentApply ?? 'auto') === 'auto') {
        ...
        if (r.missed?.length) {
          incident(sink, 'guard', 'intake-op-missed', { user_id, household_id, session_id: session.id, card_id, missed: r.missed });
        }
    ...
    if (call.meta.example_copy) {
      incident(sink, 'guard', 'example-copy', { user_id, household_id, session_id: session.id, model: call.meta.model });
    }
    ...
    if (!summaryTurn && call.note) {
      ...
      if (d.accepted) input.log.info({ user_id, note: d.note.text }, 'note-added');
      else input.log.info({ user_id, reason: d.reason, note: d.text }, 'note-skipped');
    }

    // Пул-2 №5: те саме для чату — сирий JSON у стрічку не протікає ніколи.
    if (!call.card && looksLikeModelDebris(call.reply ?? '')) {
      incident(sink, 'guard', 'chat-reply-debris', { user_id, household_id, session_id: session.id });
      call.reply = INTAKE_TOO_BIG_REPLY;
    }
    ...
    if (tenseViolation(call.reply ?? '', call.card)) {
      ...
      incident(sink, 'guard', 'tense-violation', { user_id, household_id, session_id: session.id, card_type: call.card?.type ?? null });
    }
    ...
    if (wantsRecipe && !call.card) {
      incident(sink, 'guard', 'proposal-card-missed', { user_id, household_id, session_id: session.id, model: call.meta.model });
    }
    ...
    if (houseAllergies.length) {
      ...
      if (hit.length) {
        incident(sink, 'guard', 'response-contains-allergen', { user_id, household_id, session_id: session.id, hit, model: call.meta.model });
      }
    }
    ...
    if (call.card?.type === 'cook_go') {
      ...
      const staleNotes = profileNotes.map((n) => ({ created_at: n.created_at, recipe_title: null }));
      if (same?.payload && !recipeStaleByNotes(same, staleNotes)) {
        ...
        try {
          ...
          incident(sink, 'broke', 'cook-go-model-call-failed', { user_id, household_id, session_id: session.id, err: String(err) });
          throw new ChatTurnHttpError(502, { error: 'model_unavailable' });
        }
        ...
        if (gen.recipe) {
          ...
          const { avoid } = recipeVetoHits(resolveRecipeLabels(gen.recipe, pantry), vetoIndex, (e) => incident(sink, 'guard', e.event, { user_id, household_id, session_id: session.id, ...e }), text ?? '');
          if (avoid.length) {
            ...
            if (again.recipe) {
              const left = recipeVetoHits(resolveRecipeLabels(again.recipe, pantry), vetoIndex, (e) => incident(sink, 'guard', e.event, { user_id, household_id, session_id: session.id, retry: true, ...e }), text ?? '');
              if (left.avoid.length) incident(sink, 'guard', 'veto-recipe-kept', { user_id, household_id, session_id: session.id, title: wantedTitle, avoid: left.avoid });
              gen = again;
            }
          }
        }
    ...
    if (call.card?.type === 'recipe_edit') {
      ...
      try {
        ...
        incident(sink, 'broke', 'recipe-edit-model-call-failed', { user_id, household_id, session_id: session.id, err: String(err) });
        throw new ChatTurnHttpError(502, { error: 'model_unavailable' });
      }
      await recordUsage(repo, ctx, 'recipe_gen', gen.meta, gen.calls, genStarted, turn);
    ...
    const veto = applyVeto(call, {
      ...
      log: (e: VetoLogEntry) => incident(sink, 'guard', e.event, { user_id, household_id, session_id: session.id, ...e }),
    });
    ...
    if (veto.emptied) {
      incident(sink, 'guard', 'veto-emptied', {
    ...
    if (call.card?.type === 'period') {
      ...
      if (!call.card) {
        ...
        incident(sink, 'guard', 'period-card-dropped', {
          user_id, household_id, session_id: session.id,
