...
describe('Р147/Р149 · Telegram', () => {
  ...
  it('фото → той самий store і attachment, хід із вкладенням (auto — власник 20.09), список позицій і «Скасувати»; caption — текст ходу', async () => {
    ...
    expect(input.attachmentApply).toBe('auto');
    ...
    expect(r?.messages[0]).toContain('Записав у комору · 2');
    expect(r?.messages[0]).toContain('+ молоко · 1 л');
    expect(r?.keyboard).toEqual([[{ text: 'Скасувати', data: 'undo:11111111-1111-4111-8111-111111111111' }]]);
  });
