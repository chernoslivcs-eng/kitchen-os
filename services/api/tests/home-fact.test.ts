...
describe('GET /v1/home-fact', () => {
  ...
  const build = (opts: { generate?: (facts: string) => Promise<HomeFactCall> }) => buildApp(repo, store, mailer, { homeFact: opts });
  ...
  it('текст моделі; usage записано з профілем smart (як чат)', async () => {
    ...
    const app = build({ generate }); await app.ready();
    const me = await signIn(app, mailer, 'me@example.com');
    await cooked(me.user_id, me.household_id, 'Борщ', 2);
  ...
  it('модель каже «—» (нема фактів) — рядка нема, без guard', async () => {
    const app = build({ generate: async () => live('—') }); await app.ready();
    const me = await signIn(app, mailer, 'me@example.com');
    const res = await get(app, me.cookie);
  ...
  it('помилка моделі → { text: null } і guard; задовга відповідь без межі речення → теж null', async () => {
    const app = build({ generate: async () => { throw new Error('timeout 8000ms'); } }); await app.ready();
    const me = await signIn(app, mailer, 'me@example.com');
