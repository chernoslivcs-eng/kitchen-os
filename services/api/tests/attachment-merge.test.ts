...
import { sumUsage, type AttachmentCall } from '../src/model.js';
...
const intake = (labels: string[], reply = 'Розібрав.'): AttachmentCall => ({
  ...
  calls: [{ input: 10, output: 5, cached: 1 }], meta,
});
...
describe('mergeAttachmentCalls', () => {
  ...
  it('два чеки → одна intake_diff з усіма ops, ДВА записи обліку, reply з кількістю', () => {
    ...
    // Крок А4б: два вкладення — два виклики моделі, тож і два записи обліку.
    // Досі тут стояла СУМА в одному записі: гроші сходились, а «ціна одного
    // виклику» виходила вдвічі більшою за ту, що в рахунку OpenRouter.
    expect(m.calls).toHaveLength(2);
    expect(sumUsage(m.calls)).toMatchObject({ input: 20, output: 10, cached: 2 });
  ...
  it('чек + фото страви: інтейк лишається, страва не ламає картку', () => {
    const dish: AttachmentCall = { reply: 'Гарний вигляд.', raw_kind: 'dish', card: null, calls: [{ input: 1, output: 1 }], meta };
  ...
  it('жодної картки — перша непорожня репліка, картка null', () => {
    const other: AttachmentCall = { reply: 'Не розібрав.', raw_kind: 'other', card: null, calls: [{ input: 1, output: 1 }], meta };
