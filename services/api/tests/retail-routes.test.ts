...
describe('retail routes · silpo', () => {
  ...
  it('sync-receipts: чек → auto-apply в комору, nonfood/unmatched чесно у відповіді', async () => {
    await connect();
    ...
    expect(body.nonfood.map((l: { name: string }) => l.name)).toEqual(['Папір туалетний Zewa 8 рулонів']);
    expect(body.unmatched.map((l: { name: string }) => l.name)).toEqual(["Дрова Pen'ok Початок вогню №2"]);
    expect(body.receipt).toMatchObject({ shop: 'вул. Київська, буд. 10', total: 2644 });
