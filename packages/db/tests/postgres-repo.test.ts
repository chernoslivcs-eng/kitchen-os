...
if ('skip' in backend) {
  ...
  describeRepoContract('PostgresRepo', {
    async make() {
      ...
      await pool.query('TRUNCATE token_usage, auth_challenge, auth_session, attachment, card_pending, pantry_batch, household_member, profile, household, "user" RESTART IDENTITY CASCADE');
