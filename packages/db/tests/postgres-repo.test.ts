...
if ('skip' in backend) {
  ...
  describeRepoContract('PostgresRepo', {
    async make() {
      ...
      await pool.query('TRUNCATE household_invite, app_event, token_usage, auth_challenge, auth_session, attachment, card_pending, pantry_batch, household_product, message, session, cook_run, recipe, shopping_item, eater, household_member, profile_text, profile_note, veto_index, household_event, occasion_subscription, household_occasion_catch, household, "user" RESTART IDENTITY CASCADE');
