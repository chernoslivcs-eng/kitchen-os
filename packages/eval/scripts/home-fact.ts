...
async function main() {
  ...
  const head = [
    ...
    `Правило виходу: промпт просить одне–три речення до 300 знаків лише з переданих фактів; сервер зрізає довше за 220 по межі речення, без межі або з емодзі — { text: null } (лишається шаблон); «—» = модель не знайшла фактів.`,
    '',
