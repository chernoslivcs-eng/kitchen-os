...
async function ensureProduct(
  ...
  // Каталог (кроки 2-3): key по аліасах + дефолти в тегах.
  // Межа жорстка: каталог дає властивості КЛАСУ (алергени, скоромність),
  // ніколи екземпляра — бренд/варіант/назву не чіпає.
  //
  ...
  if (cat) {
    ...
    // Скоромність — властивість класу, і каталог знає її краще за модель, тож
    // тут каталог б'є модель В ОБИДВА БОКИ, а не доливає в дірку.
    //
    // Підстава виміряна, не виведена: у розборі чека 09.09 модель написала
    // `fasting: false` на фует, тунець, камамбер і яйця — і `true` на шпинат,
    // часник, борошно та олію. Тобто в тому виклику полярність поля була
    // перевернута цілком. Покладатись на слово моделі там, де каталог знає
    // клас, у такому разі означає віддати мітку посту випадковості.
    //
    // Небезпечний тут лише один напрям: пропущене «скоромне». Мітку ⚠ПІСТ у
    // рядку комори (context.ts:178) дає АБО корінь у назві партії, АБО цей
    // тег; на восьми з тих одинадцяти рядків назва мовчить, тож хибний
    // `false` знімав останнє, що лишалось.
