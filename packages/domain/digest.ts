...
export function pickForm(input: DigestInput): DigestPick | null {
  ...
  if (alive) return { form: 4, theme: 'страва з того, що є', facts: '', button: { text: 'Рецепт', next: '/app' } };
  return null;
}
...
export function digestRequest(theme: string, facts: string): string {
  return `[СЕРВЕР] Одне речення до вечірнього нагадування. Тема: ${theme}. Факти: ${facts}. Без переліку, без порад, без питань, без звертання до обмежень і алергій.`;
}
