...
describe('загальні записи · строк консервативний', () => {
  it('швидкопсувні: строк консервативний — не довший за найкоротший видовий варіант групи (sealed)', () => {
    // v2 (21.09): таблиця розрізняє стан і обробку — копчене, фарш, субпродукти живуть
    // за власними рядками, тож із «видових варіантів» групи вони виключені.
    const EXCL = new Set(['фарш', 'субпродукти', 'домашня ковбаса', 'вуха', 'напівфабрикат', 'копчене', 'варено-копчене', 'напівкопчене', 'сирокопчене', 'вʼялене', 'готове', 'кулінарія', 'готові страви', 'консерви', 'рибні консерви']);
    for (const g of GENERIC_0915.filter((x) => x.shelfGroup)) {
      ...
      if (!grp.length) continue;   // усі видові варіанти — обробка (салямі: копчене/сирокопчене) — порівнювати нема з чим
