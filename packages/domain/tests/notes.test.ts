...
describe('серіалізація висновків для моделі', () => {
  ...
  // Було «жодного токена». M13-ROLE-VOICE п.1: card-rules.md наказує «не
  // пропонуй записати те, що вже там є» — без блока звіряти було ні з чим,
  // і модель пропонувала дублі висновків.
  it('порожньо — обидва блоки є і кажуть, що записів немає', () => {
...
describe('наміри (kind:intent)', () => {
  ...
  it('самі наміри без висновків — блок висновків не рендериться', () => {
    ...
    // Було «блок висновків не рендериться». Тепер він є, але порожній —
    // важливе тут не зникнення блока, а що намір НЕ протік у висновки:
    // намір про майбутнє, висновок — урок з минулого, змішувати не можна.
    expect(s).toContain('[ВИСНОВКИ З ГОТУВАННЯ]');
    expect(s.split('[НАМІРИ]')[0]).not.toContain('роли з креветками');
    expect(s.split('[НАМІРИ]')[1]).toContain('роли з креветками');
  });
});
