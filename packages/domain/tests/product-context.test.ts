...
describe('serializePantry × продукт дому', () => {
  ...
  it('запечатана партія без дати → «~строк≈» від зони, і НЕ точний «!Nдн»', () => {
    // Б1. Досі запечатана партія не несла строку взагалі — 245 із 246 позицій
    // модель бачила без жодної позначки часу.
    //
    // Позначка саме приблизна, і це не дрібниця: `state-facts.md` наказує
  ...
  it('ручна дата лишається точною навіть там, де зона порахувала б інакше', () => {
    // Пріоритет із Р2: `expires_at` означає «людина сказала» і бʼє таблицю.
    const b = batch({
      label: 'помідори', zone: 'fresh',
      added_at: new Date(NOW - 5 * DAY).toISOString(),
...
describe('serializePantry: кеп і відбір', () => {
  ...
  it('без запиту і квот — поведінка як була: свіжі перемагають, хвіст числом', () => {
    // Зона тут `dry` навмисно, і до Б1 вона ролі не грала. Партії в цьому
    // семплі рознесені на 130 ДНІВ, а дефолтна зона хелпера — `fridge`, який
    // живе 21 день. Відколи строк рахується від зони, така комора означає, що
    // 112 зі 130 позицій прострочені, — а холодильник, у якому півтори сотні
    // позицій лежать по чотири місяці, це не усталений стан, а неможливий.
    // У сухій шафі (540 днів) вік у 130 днів звичайний, і семпл знову міряє
    // те, заради чого писався: коли нічого не горить, кеп добирає найсвіжіші.
    const bs = many(130, (i) => ({ zone: 'dry' as const, added_at: new Date(NOW - i * DAY).toISOString() }));
    const out = serializePantry(bs, NOW, false, 'none', 120, []);
    expect(out).toContain('…і ще');
    expect(out).toContain('продукт 0');
  });

  it('коли горить майже все — першими з кепа випадають НАЙСВІЖІШІ, не залежані', () => {
    // Знахідка Б1, за межею боргу №18. Обрив у боргу описаний з боку квоти
    // залежаним, але вона гине ДРУГОЮ: ярус «решта за свіжістю» стоїть
    // четвертим і лишається ні з чим раніше. Тобто в коморі, де прострочено
    // майже все, модель перестає бачити щойно куплене.
    //
