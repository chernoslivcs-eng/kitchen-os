...
function movableOccasions(date: Date, trads: Tradition[]): Occasion[] {
  ...
  if (inRange(shift(-48), shift(-1))) out.push({
    ...
    restricts: 'жодного мʼяса, риби, молочного і яєць — ні у стравах, ні в needs. Рослинні аналоги (кокосове молоко, тахіні) можна',
...
export function serializeOccasions(now = new Date(), wishes: string[] = []): string {
  ...
  if (restricting.length) {
    parts.push('ТРИВАЄ ОБМЕЖЕННЯ (людина сама сказала, що дотримується — тверда межа, як алергія):\n'
