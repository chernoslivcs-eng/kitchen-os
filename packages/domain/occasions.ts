...
function movableOccasions(date: Date, trads: Tradition[]): Occasion[] {
  ...
  if (inRange(shift(-48), shift(-1))) out.push({
    ...
    meaning: 'Без мʼяса, молочного і яєць. У традиції це не обмеження, а привід зайти в бобові, гриби й крупи глибше, ніж зазвичай.',
...
export function serializeOccasions(now = new Date(), wishes: string[] = []): string {
  ...
  return '\n\n[СЕЗОН І СВЯТА]\n' + parts.join('\n')
    + '\nЦе привід, а не обовʼязок: згадуй лише коли доречно, одним реченням усередині відповіді.'
