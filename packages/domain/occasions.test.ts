...
describe('блок для промпта', () => {
  it('містить активне й майбутнє', () => {
    ...
    expect(s).toContain('пік овочевого сезону · до 20 вер.');
    // П1а: сезон — одним рядком, без покупок і сенсу.
  ...
  it('піст подається обмеженням, окремо від приводів', () => {
    ...
    expect(both.indexOf('· суворо')).toBeLessThan(both.indexOf('черемша'));
  });
...
describe('разова подія і тижневе правило', () => {
  ...
  it('перехід на зимовий час не зсуває день тижня', () => {
    // 25.10.2026 годинник переводять назад. Крок у мілісекундах давав
    // «вівторок» у понеділок — живий прогін календаря це й показав.
    const occ = occurrencesInRange({ t: 'weekly', dow: 2 }, at('2026-10-19'), at('2026-11-09'), []);
    for (const o of occ) expect(new Date(o.start).getDay()).toBe(2);
    expect(occ).toHaveLength(3);   // 20, 27 жовтня і 3 листопада — 10-те вже поза вікном
  });
