...
const STRONG_PRODUCT_STEMS = [
  ...
  ...['месендж', 'мессендж', 'месндж', 'меснж', 'вайбер', 'viber', 'вотсап', 'ватсап', 'whatsapp'],
];
// 2б) Месенджери — карта вмикається від самого слова, без питального: «а
// телеграм?» — уже питання. Ширші слова («профіль», «календар») сюди не
// беремо: «профіль» одним словом — не питання про додаток (фікстура NO).
const MESSENGER_STEMS = ['телеграм', 'telegram', 'месендж', 'мессендж', 'месндж', 'меснж', 'вайбер', 'viber', 'вотсап', 'ватсап', 'whatsapp'];
...
export function isProductQuestion(text: string): boolean {
  ...
  // 14.09 (лог: «А телеграм?», «А месенджер можна підключити?» → карта не
  // підмішалась, модель сказала «нема»): слово лише-про-додаток вмикає карту
  // саме, без питального слова — коротке «а телеграм?» теж питання.
  if (ws.some((w) => matchesStem(w, MESSENGER_STEMS))) return true;
